...
function showAuthAnimation(username, type) {
    ...
    const message = isLogin ? `שלום, ${username} 👋` : `ביי ביי, ${username} 😢`;
    const overlay = document.createElement('div');
    overlay.id = 'auth-anim-overlay';
    const starsHTML = Array.from({ length: 120 }, () => {
        ...
        return `<div class="auth-star" style="width:${size}px;height:${size}px;left:${x}%;top:${y}%;background:${color};animation-delay:${delay}s;animation-duration:${dur}s;"></div>`;
    }).join('');
    overlay.innerHTML = `
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => overlay.classList.add('auth-anim-visible')));
    setTimeout(() => {
...
function updateAuthUI(username) {
    ...
    if (username) {
        ...
        const avatarUrl = localStorage.getItem('currentAvatar');
        if (avatarUrl) {
            btn.innerHTML = `<img src="${avatarUrl}" alt="${username}">`;
        } else {
...
function toggleUserMenu() {
    ...
    panel.classList.contains('hidden') ? openUserPanel() : closeUserPanel();
}
function openUserPanel()  { document.getElementById('user-panel').classList.remove('hidden'); }
function closeUserPanel() { document.getElementById('user-panel').classList.add('hidden'); }
...
function openAuthModal()  { document.getElementById('auth-modal').classList.remove('hidden'); document.getElementById('auth-msg').innerText = ''; }
...
async function submitAuth(action) {
    ...
    if (action === 'login') {
        ...
        if (password !== pass2) { msgDiv.style.color = '#f44336'; msgDiv.innerText = 'הסיסמאות אינן תואמות!'; return; }
    }
    if (!username || !password) { msgDiv.style.color = '#f44336'; msgDiv.innerText = 'נא למלא את כל השדות.'; return; }
    try {
        const res  = await fetch(`${API_BASE}/api/auth/${action}`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({username,password}) });
        ...
        if (res.ok) {
            ...
            localStorage.removeItem('currentAvatar');
            updateAuthUI(user);
            setTimeout(() => { closeAuthModal(); showAuthAnimation(user, 'login'); }, 600);
        } else {
